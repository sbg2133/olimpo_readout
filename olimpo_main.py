...
class roachInterface(object):
    ...
    def __init__(self):
    	self.center_freq = 400.0  # this is the LO frequency in MHz
	self.v1 = valon_synth.Synthesizer('/dev/ttyUSB0')
	#self.v1.set_frequency(8,self.center_freq,0.01)
        self.ip = '192.168.40.95'
...
# Builds the DDS look-up-table from I and Q given by freq_comb. freq_comb is called with the sample rate equal to the sample rate for a single FFT bin. There are two bins returned for every fpga clock, so the bin sample rate is 256 MHz / half the fft length  
	...
	self.fpga.write_int('pps_start', 1)
        count = 0
        f_chan = open('/home/olimpo/data/ch' + str(chan), 'ab')
        f_time = open('/home/olimpo/data/time' + str(chan), 'ab')
	while count < Npackets:
	    ts = time.time()
            packet = self.s.recv(8234) # total number of bytes including 42 byte header
            data = np.fromstring(packet[42:],dtype = '<i').astype('float')
	...
	os.mkdir(save_path)
	self.v1.set_frequency(8, center_freq/1.0e6, 0.01)
        span = self.pos_delta
	...
	if write:
		...
		self.v1.set_frequency(8,center_freq / (1.0e6), 0.01) # LO
	self.plot_vna(save_path)
	#self.find_kids_olimpo.main(path)
	return 

    def target_sweep(self, center_freq = 400.0e6, save_path = './sweeps/target', write = True, sweep = False):
	...
	#center_freq = (np.max(self.target_freqs) + np.min(self.target_freqs))/2.
        self.bb_target_freqs = ((self.target_freqs*1.0e6) - 200.0e6)
	...
	upconvert = np.sort((self.bb_target_freqs + 200e6)/1.0e6)
    ...
    def sweep_lo(self, Npackets_per = 100, channels = None, center_freq = 400.0e6, span = 2.0e6, save_path = '/sweeps/vna'):
        for freq in self.sweep_freqs:
            print 'Sweep freq =', freq/1.0e6
            if self.v1.set_frequency(8, freq/1.0e6, 0.01): 
	...
	LO_freq = (self.center_freq/2)*1.0e6
	Npackets = np.int(time_interval * self.accum_freq)
