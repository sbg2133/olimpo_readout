...
class FirmwareSnaps(object):
    	...
    	def __init__(self):
		self.fpga = casperfpga.katcp_fpga.KatcpFpga("192.168.40.95",timeout=120.)
		self.dds_shift = 305
		self.accum_len = 2**21  
		self.fft_len = 1024
	...
	def plotADC(self):
		...
		plt.xlim(0,1024)
		plt.ylim(-2.5,2.5)
		plt.yticks(np.arange(-2.5, 2.5, 0.3))
	...
	def plotFFT(self):
		...
		line1, = plot1.plot(np.linspace(0,512.0, 4096), np.zeros(self.fft_len), '#FF4500')
		#line1.set_linestyle('None')
		#line1.set_marker('.')
		plt.xlabel('freq (MHz)',fontsize = 20)
		plt.ylabel('Amplitude [mV]',fontsize = 20)
		plt.title('Pre-DDS FFT',fontsize = 20)
		plt.xticks(np.arange(0,512.0,10))
		plt.xlim((0,512.0))
		plt.tight_layout()
		plt.grid()
		plt.show(block = False)
		...
		while(count < stop):
			...
			fft_snap = (np.fromstring(self.fpga.read('fft_snap_bram',(2**11)*8),dtype='>i2')).astype('float')
			I0 = fft_snap[0::4]
			Q0 = fft_snap[1::4]
			I1 = fft_snap[2::4]
			Q1 = fft_snap[3::4]
